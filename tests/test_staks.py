...
def test_total_spent():
    assert run_query("tasks/query_02.sql")[0][0] == pytest.approx(2297.47, 0.01)

def test_avg_price_by_category():
    result = dict(run_query("tasks/query_03.sql"))
    assert result["Electronics"] == pytest.approx(287.49, 0.01)
    assert result["Books"] == pytest.approx(12.17, 0.01)
    assert result["Clothing"] == pytest.approx(42.50, 0.01)
